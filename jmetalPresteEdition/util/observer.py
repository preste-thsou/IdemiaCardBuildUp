...
class WriteFrontToFileObserver(Observer):
    ...
    def __init__(self, output_directory: str, print_to_file = False) -> None:
    ...
    def get_numpy_arrays(self, solutions):
        fun_list = []
        for solution in solutions:
            fun_list.append([function_value for function_value in solution.objectives])
        #print('\n\nnp.array(fun_list) - >', np.array(fun_list) )
        return np.array(fun_list, dtype = np.float)
    ...
    def update(self, *args, **kwargs):
        ...
        if solutions:
            ...
            solutions_np = self.get_numpy_arrays(solutions)
            front = get_non_dominated_solutions(solutions)
            front_np = self.get_numpy_arrays(front)

            solutions_std = np.std(solutions_np, axis=0) 
            solutions_mean = np.mean(solutions_np, axis=0)
